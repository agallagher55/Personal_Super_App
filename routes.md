...
| `/finance` | `html/finance.html` | `static/finance/js/ticker.js`, `static/finance/js/dashboard.js` | Net worth dashboard template (cash, investments, bitcoin, debt, lines of credit — each with a % of section total) rendered from sample data in `static/finance/js/dashboard-data.js`, plus a sidebar watchlist (Bitcoin, gold, WTI crude, S&P 500). No backend route behind the dashboard yet — see `finance/ARCHITECTURE.md` for the plan to replace the sample data with a real Plaid-backed sync. |
