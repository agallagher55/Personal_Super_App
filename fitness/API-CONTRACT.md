...
There is no `food` metric. The Google Health API does not expose a
`nutrition` data type at all, so the sync cannot produce one and asking for
it only turns every refresh into a misleading partial-sync warning. A Food
section needs a different source entirely; see "Nutrition data
availability" in `google_health.md` before adding one back.
